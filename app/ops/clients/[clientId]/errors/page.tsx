/**
 * /ops/clients/[clientId]/errors — Client-scoped workflow errors console.
 */
...
export default async function ClientErrorsPage({
  ...
  const client = clientData as Pick<Client, 'id' | 'name' | 'slug' | 'brand_color'> | null

  const errors = await getWorkflowErrors(100, clientId)
  ...
  return (
    <div className="min-h-screen bg-[var(--brand-bg)]">
        ...
        <div className="max-w-5xl mx-auto w-full flex items-center gap-4">
          ...
          {client && (
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-base font-semibold text-[var(--brand-text)]">{client.name} — Errors</h1>
                <p className="text-xs text-[var(--brand-muted)]">Workflow errors for this client</p>
              </div>
            </div>
          )}
        </div>
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-6">
        <Card>
          <CardHeader>
            <CardTitle className="text-base">Workflow Errors</CardTitle>
            <CardDescription>
              Errors ingested via n8n for {client?.name ?? 'this client'}
            </CardDescription>
          </CardHeader>
          <CardContent className="p-0">
            {errors.length === 0 ? (
              <div className={polish.emptyState}>
                ...
                <div>
                  <p className="text-sm font-medium text-[var(--brand-text)]">No errors for this client</p>
                  <p className="text-xs text-[var(--brand-muted)] mt-0.5">
                    Include <code className="font-mono text-[10px]">clientId</code> in the ingest payload to scope errors.
                  </p>
                </div>
              </div>
            ) : (
