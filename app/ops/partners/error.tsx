...
export default function PartnersError({
  ...
  useEffect(() => {
    console.error('[partners] Unhandled error:', error)
  }, [error])

  return (
    <div className="flex min-h-screen items-center justify-center p-6 bg-[var(--brand-bg)]">
      <div className="max-w-md text-center">
        ...
        <h2 className="text-lg font-semibold text-[var(--brand-text)] mb-2">
          Partner Console Error
        </h2>
        <p className="text-sm text-[var(--brand-muted)] mb-6">
          An error occurred loading the partner console.
          {error.digest && (
