-- ============================================================================
-- Migration 032: Ops Financial Profiles + Payment Logs + Financial Events
--
-- Creates the three missing financial tables used by the ops dashboard.
-- Based on sql/020_ops_financial_profiles.sql, adapted for production schema.
--
-- FK convention: These tables use `client_id` referencing `tenants(id)`.
-- The app's mutation layer (lib/ops-financials/mutations.ts) writes `client_id`,
-- and the query layer (lib/ops-financials/query.ts) filters by `client_id`.
-- The FK targets `tenants` because that's the live table name in production.
--
-- CAC/LTV data lives in client_unit_economics (migration 024) — not here.
